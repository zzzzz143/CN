...
#define MAX_BUFFER_SIZE 4096 // 数据包最大长度
...
void SavePacketToFile(const Packet& packet, const std::string& filename) {
    ...
    size_t bytesWritten = fwrite(packet.msg, sizeof(char), packet.filelength, file);
    if (bytesWritten < packet.filelength) {
        // 如果写入的字节数小于要写入的字节数，可能发生了错误
        SetConsoleTextAttribute(hConsole, FOREGROUND_RED);
        std::cerr << "[Error writing to file.]" <<"Expected:"<<packet.filelength<<"Actual:"<<bytesWritten<<std::endl;
    }
    fclose(file); // 关闭文件
}
...
DWORD WINAPI ReceiveDataThread(LPVOID lpParam) {     //用于接收数据
    ...
    while (true) {
        ...
        if(GetTickCount() - startTime > TIMEOUT){ //超时
            ...
            cout<<"[Time Out]";
            SetConsoleTextAttribute(hConsole, 0x07);
            cout<<"Resend ACK:"<<expectedseq<<endl;
        ...
        if (result == SOCKET_ERROR) {}   //表示没数据可以接收
            ...
            switch(receivedPacket.flag){  //根据标志位判断包的类型---

                case 0x04: //Data  //接收到了数据包，说明服务端的上一个发的ACk成功收到了，因此发送序号可以增加
                    
                    if(receivedPacket.msgseq != expectedseq){    //真正收到的与期望收到的不一致，回复按序正确接收的最高序号分组
                        std::unique_lock<std::mutex> lock(consoleMutex); // 加锁
                        SetConsoleTextAttribute(hConsole, FOREGROUND_RED);
                        cout<<"[Wrong Sequence Number]";
                        SetConsoleTextAttribute(hConsole, 0x07);
                        cout<<"Received:"<<receivedPacket.msgseq<<" Expected:"<<expectedseq<<endl;
                        IsSendACK = true;  //IsSendACK设置为true后会重发ACK
                        lock.unlock(); // 解锁
                        continue;   //接着等待
                    }
                        ...
                        cout<<"[Data Received]";
                        SetConsoleTextAttribute(hConsole, 0x07);
                        cout<<" Seq:"<<receivedPacket.msgseq<<" Len:"<<receivedPacket.filelength<<endl;
                        lock.unlock(); // 解锁
                        
                        SavePacketToFile(receivedPacket, "./COPY-"+filename); // 保存数据包到文件
                        expectedseq = receivedPacket.msgseq + 1; //更新期望的序列号
                        msgseq = msgseq + 1;/////更新msgseq，因为收到了数据包说明上一个ACK已经发送成功了
                    }
                ...
                case 0x08: //FIN
                    ...
                    expectedseq = receivedPacket.msgseq + 1; //更新期望的序列号
                    msgseq = msgseq + 1;/////因为收到了数据包说明上一个ACK已经发送成功了->更新msgseq
                    IsThreadOver = true;
                ...
                case 0x10: //文件名
                    ...
                    SetConsoleTextAttribute(hConsole, 0x07);
                    cout<<"Seq:"<<receivedPacket.msgseq<<endl;
...
DWORD WINAPI SendDataThread(LPVOID lpParam) {
    ...
    SOCKET socket = params->socket;
    struct sockaddr_in clientAddr = params->clientAddr;
    while(true){   //在收到失序分组/超时时发送ACK，其余时间一直在等待累计确认
        if(IsThreadOver == true)break;
        if(IsSendACK == true){
            // 准备要发送的数据包
