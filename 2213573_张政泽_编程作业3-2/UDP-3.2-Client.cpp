...
int main(){
   ...
   //---------------------------------------------------------------------------------------------- 
    ...
    base = nextseqnum = msgseq;///初始化base和nextseqnum
   ...
   //接下来是不断地传输数据
    ...
    while(true){
        ...
        if (sendResult == SOCKET_ERROR) {
            ...
            cout<<"   WindowLeft:";
            cout<<WindowSize-(nextseqnum-base)<<endl;
            lock.unlock(); // 解锁
        }
        ...
        if(IsReceived){   //收到了ACK，更新窗口
            ...
            if(packet.flag == 0x00){
                ...
                cout<<"InOut Rate:"<<totalSend*4096*CLOCKS_PER_SEC/(clock()-FileTransmitTime)<<"byte/s"<<endl;
                IsThreadOver = true;
                break;
            }
            continue;
        ...
        //没收到ACK,接着发下一个
        if(nextseqnum - base < WindowSize){ //窗口未满，可以继续发送
            msgseq++;  //准备下一个
            nextseqnum++;
            packet = packets[nextseqnum];
            if(packet.flag == 0x00){
